...
HR_HB = 5      # 心跳计数（从站写，只读）
HR_MANUAL = 8  # 手操值（主站写，手动模式下生效；HR6/HR7 为 DI/DO 预留区）
...
def scale_to_reg(value: float) -> int:
    ...
    结果钳位到 16 位无符号寄存器量程 [0, 65535]（代码评审修复项）：
    正常工艺值（PV 满量程 10 mg/L -> 1000）远不会触及该边界，钳位是防
    "异常值经 %65536 回绕成 655+ mg/L 假读数"的第二层防护——第一层在
    过程模型测量端（process/plant.py 噪声叠加后截断）。
    """
    reg = int(round(value * 100))
    if reg < 0:
        return 0
    if reg > 65535:
        return 65535
