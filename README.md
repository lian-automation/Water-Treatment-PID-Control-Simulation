...
- **控制层**：增量式 PID——输出限幅、增量钳位抗积分饱和（含退饱和
  诊断量）、测量死区、手/自动无扰切换、微分先行 + 不完全微分 +
  微分限幅、参数在线修改；
- **整定**：两点法响应曲线辨识（K/T/τ）→ Z-N 公式 → 工程修正，
  保守参数与整定参数同场景对比；
- **联调层**：pymodbus Modbus/TCP 从站（双 Unit，HR0~HR5 寄存器映射
  + HR8 手操值寄存器，心跳 + 报警位），附主站自测脚本；
...
2. **手/自动**：切手动时弹窗输入初始手操值（默认当前输出，保证无扰）；
   手动状态下"手操下发"可用。单机与联调模式均可用——联调模式经
   HR8 手操值寄存器下发（从站手动模式下边沿生效）；
...
| 项目 | 结果 |
|---|---|
| Modbus 主站自测 | **17/17 项 PASS**（SP 下发回读、手自动切换、"闭环在调"OP 响应、心跳、只读写保护）；冷启动连跑 5 次 + 预热复跑 2 次全过。复现：起从站后另开终端跑主站自测，退出码 0=全过（命令见 §7；判据设计见 `plc_link/modbus_client_test.py` 模块注释与验收清单 5C） |
...
- 抗积分饱和：增量式固有钳位——输出增量始终累加在「上拍已限幅输出」
  上，退出饱和无拖尾；另设积分退饱和诊断量（按 1/Tt 回灌截断差，
  仅显示不参与控制律）；
- 微分先行（避免 SP 阶跃微分冲击）+ 不完全微分（Tdf=Td/N，本项目对照
  实验数据见 `docs/整定对比报告.md` 附录：噪声大的回路 N=2 优于 N=5，
  N 可经构造参数按实例配置）+ 微分限幅（±10% 量程/拍）；
...
![监控看板运行实况](docs/img/dashboard.png)

*Web 看板单机模式运行实况：加药/曝气双回路实时趋势（SP/PV/OP）、SP 下发、手/自动无扰切换、PID 在线整定与报警列表（激活/恢复），全部为真实运行数据渲染。*
...
覆盖：双 Unit 连接、心跳增长、PV/OP 量程、HR3 切手动回读、HR0 SP 阶跃回读、**"闭环在调"OP 响应**、只读写保护（HR1/2/4/5 外部写拒绝、批量写触及只读区整段拒绝）等，全过退出码 0。其中"闭环在调"判据已改为验证 **OP 对 SP 阶跃的即时响应**（增量式比例增量与阶跃同拍生效、不经过过程滞后；双窗口各 3 次采样均值差 ≥ 2.5，为寄存器量化步长的 250 倍）——旧判据"8s 内 PV 向新 SP 靠拢"落在加药回路 τ=30s 纯滞后死区内、不可复现。冷启动连跑 5 次 + 预热复跑 2 次全过（验收清单 5C）。
...
可以。噪声种子固定，`python run_test.py` / `python -m tune.tuner` 在干净环境对入库报告**字节级复现**（验证记录：测试报告除时间戳 diff 全等、整定报告字节级一致）；`docs/*.html` 为脚本再生工件，随代码更新重跑即可。
